...
const INITIAL_DATABASE: DatabaseState = {
  ...
  users: [
    ...
    { id: 'usr-3', username: 'aulia', nama: 'Aulia Rohmah, S.Pd,.MM', role: UserRole.WALI_KELAS, email: 'aulia.rohmah@sekolah.sch.id', isActive: true },
    ...
    { id: 'usr-5', username: 'novita', nama: 'Novita Kusuma Wardhani, S.Pd', role: UserRole.WALI_KELAS, email: 'novita@sekolah.sch.id', isActive: true },
    { id: 'usr-6', username: 'dwisusanti', nama: 'Dwi Susanti, S.Pd', role: UserRole.WALI_KELAS, email: 'dwi@sekolah.sch.id', isActive: true },
  ],
  tahunPelajaran: [
  ...
  kelas: [
    ...
    // Kelas 9-1 s.d. 9-11
    { id: 'kl-23', namaKelas: 'Kelas 9-1', waliKelasId: 'usr-6' },
    { id: 'kl-24', namaKelas: 'Kelas 9-2', waliKelasId: 'usr-6' },
    { id: 'kl-25', namaKelas: 'Kelas 9-3', waliKelasId: 'usr-6' },
    { id: 'kl-26', namaKelas: 'Kelas 9-4', waliKelasId: 'usr-6' },
    { id: 'kl-27', namaKelas: 'Kelas 9-5', waliKelasId: 'usr-6' },
    { id: 'kl-28', namaKelas: 'Kelas 9-6', waliKelasId: 'usr-6' },
    { id: 'kl-29', namaKelas: 'Kelas 9-7', waliKelasId: 'usr-6' },
    { id: 'kl-30', namaKelas: 'Kelas 9-8', waliKelasId: 'usr-6' },
    { id: 'kl-31', namaKelas: 'Kelas 9-9', waliKelasId: 'usr-6' },
    { id: 'kl-32', namaKelas: 'Kelas 9-10', waliKelasId: 'usr-6' },
    { id: 'kl-33', namaKelas: 'Kelas 9-11', waliKelasId: 'usr-6' },
  ],
  siswa: [
    {
      id: 'sis-1',
  ...
  pelanggaran: [
    { id: 'pel-1', siswaId: 'sis-3', tanggal: '2026-06-15', jenisPelanggaran: 'Merokok di area sekolah', kategori: 'Berat', poin: 75, guruPelapor: 'Aulia Rohmah, S.Pd,.MM', tindakLanjut: 'Pemanggilan Orang Tua', status: 'Proses' },
    ...
    { id: 'pel-3', siswaId: 'sis-3', tanggal: '2026-06-25', jenisPelanggaran: 'Bolos sekolah pada jam pelajaran produktif', kategori: 'Sedang', poin: 30, guruPelapor: 'Aulia Rohmah, S.Pd,.MM', tindakLanjut: 'Konseling Individu & SP 1', status: 'Belum Ditindak' },
...
export function sanitizeDatabaseState(parsed: any): { sanitized: DatabaseState; migrated: boolean } {
  ...
  parsed.users = parsed.users.map((u: any) => {
    ...
    if (u.id === 'usr-3') {
      hasUsr3 = true;
      if (u.nama !== 'Aulia Rohmah, S.Pd,.MM' || u.role !== UserRole.WALI_KELAS || u.username !== 'aulia') {
        u.nama = 'Aulia Rohmah, S.Pd,.MM';
        u.username = 'aulia';
        u.email = 'aulia.rohmah@sekolah.sch.id';
    ...
    if (u.id === 'usr-5') {
      hasUsr5 = true;
      if (u.nama !== 'Novita Kusuma Wardhani, S.Pd' || u.role !== UserRole.WALI_KELAS) {
        u.nama = 'Novita Kusuma Wardhani, S.Pd';
        u.role = UserRole.WALI_KELAS;
        u.email = 'novita@sekolah.sch.id';
        migrated = true;
      }
    }
    if (u.id === 'usr-6') {
      hasUsr6 = true;
      if (u.nama !== 'Dwi Susanti, S.Pd' || u.role !== UserRole.WALI_KELAS) {
        u.nama = 'Dwi Susanti, S.Pd';
        u.role = UserRole.WALI_KELAS;
        u.email = 'dwi@sekolah.sch.id';
        migrated = true;
      }
    }
  ...
  if (!hasUsr3) {
    parsed.users.push({ id: 'usr-3', username: 'aulia', nama: 'Aulia Rohmah, S.Pd,.MM', role: UserRole.WALI_KELAS, email: 'aulia.rohmah@sekolah.sch.id', isActive: true });
    migrated = true;
  }
  if (!hasUsr5) {
    parsed.users.push({ id: 'usr-5', username: 'novita', nama: 'Novita Kusuma Wardhani, S.Pd', role: UserRole.WALI_KELAS, email: 'novita@sekolah.sch.id', isActive: true });
    migrated = true;
  }
  if (!hasUsr6) {
    parsed.users.push({ id: 'usr-6', username: 'dwisusanti', nama: 'Dwi Susanti, S.Pd', role: UserRole.WALI_KELAS, email: 'dwi@sekolah.sch.id', isActive: true });
    migrated = true;
  }
  ...
  parsed.logAktivitas = parsed.logAktivitas.map((l: any) => {
    ...
    if (l.namaUser === 'Ahmad Dahlan, S.Pd.') {
      l.namaUser = 'Aulia Rohmah, S.Pd,.MM';
      migrated = true;
    }
    return l;
  });
  ...
  parsed.pelanggaran = parsed.pelanggaran.map((p: any) => {
    if (p && p.guruPelapor === 'Ahmad Dahlan, S.Pd.') {
      p.guruPelapor = 'Aulia Rohmah, S.Pd,.MM';
      migrated = true;
    }
    return p;
  });
